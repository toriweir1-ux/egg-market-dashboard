...
let state = {
  ...
  series: {},
  avianInfluenza: { monthly: [], byState: [], error: 'Not yet fetched' },
};
...
async function fetchAvianInfluenza() {
  try {
    ...
    const byState = transform.summarizeHpaiByState(rows);
    return { monthly: summary.monthly.slice(-TREND_HISTORY_YEARS * 12), byState, error: null };
  } catch (err) {
    return { monthly: [], byState: [], error: err.message };
  }
}
