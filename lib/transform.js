...
/**
 * Aggregate all confirmed detections by state (all-time, not windowed —
 * useful for "which states have been hit hardest overall" rather than a
 * recent-months view). Sorted by detections, descending.
 */
function summarizeHpaiByState(rows) {
  const byState = new Map();
  for (const row of rows) {
    const state = (row.State || '').trim();
    if (!state) continue;
    const bucket = byState.get(state) || { state, detections: 0, birdsAffected: 0 };
    bucket.detections += 1;
    bucket.birdsAffected += parseBirdsAffected(row['Birds Affected']);
    byState.set(state, bucket);
  }
  return Array.from(byState.values()).sort((a, b) => b.detections - a.detections);
}
